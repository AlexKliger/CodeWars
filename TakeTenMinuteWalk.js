...
Note: you will always receive a valid array (string in COBOL) containing a random assortment
of direction letters ('n', 's', 'e', or 'w' only). It will never give you an empty array
(that's not a walk, that's standing still!).
*/

function isValidWalk(walk) {
    // Immediately return false if the walk has more than 10 elements since each walk must be less than 10 minutes.
    if(walk.length > 10) return false
    
    // Every direction is assigned a coordinate number.
    const directions = {
      'n': [0, 1],
      's': [0, -1],
      'w': [-1, 0],
    ...
    // Find the sum of each respective axis.
    const sum = [0, 0]
    walk.forEach((dir) => {
      sum[0] += directions[dir][0]
      sum[1] += directions[dir][1]
    })
    
    // Return true if the sum of each axis is 0.
